...
/*
  ...
  std::vector<snu::KElectron> testel = GetElectrons(true, true, "ELECTRON_MVA_TIGHT");
  ...
  if(testmuon.size()==2 && testelectron.size()==1){
    ...
    snu::KMuon mu2 = testmuon.at(1);
    snu::KElectron el = testelectron.at(0);

    bool mlllOnZ = fabs( (mu1+mu2+el).M() - 91.1876 ) < 10.;
    bool mllOffZ = fabs( (mu1+mu2).M() - 91.1876 ) > 15.;
    FillHist("TEST_ZGamma_2mu1el_Electron_GetType", el.GetType(), 1., 0., 50., 50);
    bool conv = el.MCIsExternalConversion();
    if(el.MCIsExternalConversion()) FillHist("TEST_ZGamma_2mu1el_Electron_GetType_MCIsExternalConversion", el.GetType(), 1., 0., 50., 50);
    if(el.MCIsFromConversion()) FillHist("TEST_ZGamma_2mu1el_Electron_GetType_MCIsFromConversion", el.GetType(), 1., 0., 50., 50);

    if(conv && mlllOnZ && mllOffZ){

      std::vector<snu::KTruth> truthColl;
      eventbase->GetTruthSel()->Selection(truthColl);
      
      if(el.MCTruthIndex()>=2){
        double reco_E = el.E();
        double gen_E = truthColl.at(el.MCTruthIndex()).E();
        FillHist("TEST_ZGamma_2mu1el_Electron_MatchedPdgId", truthColl.at(el.MCTruthIndex()).PdgId(), 1., -100., 100., 200);
        FillHist("TEST_ZGamma_2mu1el_Electron_E_diff", reco_E-gen_E, 1., -100., 100., 200);
        FillHist("TEST_ZGamma_2mu1el_Electron_E_reldiff", (reco_E-gen_E)/reco_E, 1., -3., 3., 600);
      }

    }
  }

  if(testmuon.size()==3 && testelectron.size()==0){
    snu::KMuon mu1 = testmuon.at(0);
    snu::KMuon mu2 = testmuon.at(1);
    snu::KMuon mu3 = testmuon.at(2);
    if( fabs(mu1.Charge()+mu2.Charge()+mu3.Charge())==3 ) return;

    double m12 = (mu1+mu2).M();
    double m13 = (mu1+mu3).M();
    double m23 = (mu2+mu3).M();

    bool mlllOnZ = fabs( (mu1+mu2+mu3).M() - 91.1876 ) < 10.;
    bool mllOffZ = true;
    if(mu1.Charge()!=mu2.Charge()) mllOffZ &= ( fabs( (mu1+mu2).M() - 91.1876 ) > 15. );
    if(mu1.Charge()!=mu3.Charge()) mllOffZ &= ( fabs( (mu1+mu3).M() - 91.1876 ) > 15. );
    if(mu2.Charge()!=mu3.Charge()) mllOffZ &= ( fabs( (mu2+mu3).M() - 91.1876 ) > 15. );
    ...
    if(mlllOnZ && mllOffZ){
      ...
      for(unsigned c=0;c<3;c++){
        if(testmuon.at(c).GetType()==8 && testmuon.at(c).MCTruthIndex()>=2){
          n_photon++;
          double reco_E = testmuon.at(c).E();
          double gen_E = truthColl.at(testmuon.at(c).MCTruthIndex()).E();
          FillHist("TEST_ZGamma_3mu0el_Muon_E_diff", reco_E-gen_E, 1., -100., 100., 200);
          FillHist("TEST_ZGamma_3mu0el_Muon_E_reldiff", (reco_E-gen_E)/reco_E, 1., -3., 3., 600);
        }
      }
      FillHist("TEST_ZGamma_3mu0el_Muon_n_photon", n_photon, 1., 0., 4., 4);
...
/*
    ...
    std::vector<snu::KElectron> electrontriLooseColl = GetElectrons(true, true, "ELECTRON_MVA_FAKELOOSE");
    for(unsigned int i=0; i<electrontriLooseColl.size(); i++){
      ...
      if(this_electron.MCIsFromConversion()){
        ...
        if(eventbase->GetElectronSel()->ElectronPass(this_electron,"ELECTRON_MVA_TIGHT")){
...
*/
  ...
  //==== diboson, but hadronic decays
    ...
    electrontriLooseColl = GetElectrons(false, false, "ELECTRON_MVA_FAKELOOSE");
...
/*
  ...
  if(k_sample_name.Contains("ZG")){
    ...
    std::vector<snu::KElectron> allel = GetElectrons(false, true, "ELECTRON_MVA_TIGHT");
    bool isthisit=false;
...
*/
  ...
  if(isThreeMuon){
    ...
    FillCLHist(hntrilephist, "cut_MuMuMu_Presel", eventbase->GetEvent(), muontriLooseColl, electrontriLooseColl, jetColl_hn, weight);

    if( isLowMass ){
      FillCLHist(hntrilephist, "cut_MuMuMu_low", eventbase->GetEvent(), muontriLooseColl, electrontriLooseColl, jetColl_hn, weight);
      FillCutFlow("LowMass", 1.);
    }

    if( isHighMass ){
      FillCLHist(hntrilephist, "cut_MuMuMu_high", eventbase->GetEvent(), muontriLooseColl, electrontriLooseColl, jetColl_hn, weight);
    ...
    FillCLHist(hntrilephist, "cut_SSSF_MuMuE_Presel", eventbase->GetEvent(), muontriLooseColl, electrontriLooseColl, jetColl_hn, weight);
