...
float AnalyzerCore::Get_DataDrivenWeight_Trilep(bool geterr, vector<snu::KMuon> k_muons, vector<snu::KElectron> k_electrons, int n_muons, int n_electrons){

  Message("In Get_DataDrivenWeight_Trilep", DEBUG);
  float lll_weight = 0.;

  if( k_muons.size() != n_muons || k_electrons.size() != n_electrons ){
    Message("[Get_DataDrivenWeight_Trilep] number of lepton is wrong..", ERROR);
    return 0.;
  }
  ...
  for(unsigned int i=0; i<k_muons.size(); i++){
    if(k_muons.at(i).RelIso04() < 0.1) isT.push_back(true);
    else isT.push_back(false);
  }
  for(unsigned int i=0; i<k_electrons.size(); i++){
    if( fabs(k_electrons.at(i).Eta()) < 1.479 ){
      if( k_electrons.at(i).PFRelIso(0.3) < 0.0354 ) isT.push_back(true);
      else isT.push_back(false);
    }
    else{
      if( k_electrons.at(i).PFRelIso(0.3) < 0.0646 ) isT.push_back(true);
      else isT.push_back(false);
    }
  }
  ...
  lll_weight =m_fakeobj->get_trilepton_eventweight(geterr, muons, electrons, isT);
