...
void trilepton_mumumu_ntp_FR_method::ExecuteEvents()throw( LQError ){
  ...
  //float muon_id_iso_sf= MuonScaleFactor(BaseSelection::MUON_POG_TIGHT, muontriTightColl, 0); //FIXME should add sf uncertainty
  ...
  for(int it_sys=0; it_sys<=N_sys; it_sys++){
    ...
    //==== Muon
    std::vector<snu::KMuon> muontriLooseColl, muontriTightColl;
    if(this_syst == "MuonEn_up"){
      for(unsigned int i=0; i<muontriVLooseColl_lowestPtCut.size(); i++){
        ...
        if( this_muon.Pt() >= 10. && new_RelIso < 0.1 ) muontriTightColl.push_back( this_muon );
      }
    }
    ...
    //==== normal muons
      ...
      for(unsigned int i=0; i<muontriVLooseColl_lowestPtCut.size(); i++){
        ...
        if( this_muon.Pt() >= 10. && this_muon.RelIso04() < 0.1 ) muontriTightColl.push_back( this_muon );
      }
    }
...
double trilepton_mumumu_ntp_FR_method::get_FR(snu::KParticle muon, bool geterror){
